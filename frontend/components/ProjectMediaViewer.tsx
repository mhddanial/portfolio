...
export default function ProjectMediaViewer({ body, gallery, title }: ProjectMediaViewerProps) {
    ...
    const bodyImages = useMemo(() => {
        ...
        return body
            ...
            .map((block) => ({
                ...
                _key: block._key,
                url: urlFor(block).url(), // get the base URL for comparison
            }));
    }, [body]);

    // Format gallery images
    const galleryImagesWithUrl = useMemo(() => {
        if (!gallery) return [];
        return gallery.map((img) => ({
            ...img,
            url: img.asset?.url || urlFor(img).url(),
        }));
    }, [gallery]);

    // Combine all images
    const allImages = useMemo(() => [...bodyImages, ...galleryImagesWithUrl], [bodyImages, galleryImagesWithUrl]);
            ...
            {body && body.length > 0 && (
                <section className="pb-16 md:pb-20">
                    <div className="container max-w-6xl mx-auto px-6">
                        <PortableTextRenderer value={body} onImageClick={openLightbox} />
                    </div>
                </section>
            )}
                ...
                <section className="pb-16 md:pb-20">
                    <div className="container max-w-6xl mx-auto px-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {galleryImagesWithUrl.map((img, i) => (
                                <figure key={img._key || i} className="group cursor-pointer" onClick={() => openLightbox(img.url)}>
                                    <div className="relative aspect-video rounded-xl overflow-hidden border border-border bg-secondary/20">
                                        <Image
                                            src={`${img.url}?w=800&q=85`}
                            ...
                            <AnimatePresence mode="wait">
                                ...
                                >
                                    <div className="relative w-full h-full min-h-[50vh] pointer-events-auto" onClick={(e) => e.stopPropagation()}>
                                        <Image
                                            src={`${allImages[selectedIndex].url}?w=1600&q=90`}
                                            alt={allImages[selectedIndex].alt || `${title} screenshot ${selectedIndex + 1}`}
                                            fill
